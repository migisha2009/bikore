...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    const member = await pool.query('SELECT * FROM members WHERE group_id=$1 AND user_id=$2 AND status=$3', [req.params.id, req.user.id, 'active']);
    ...
    if (activeCycle) {
      const contrib = await pool.query('SELECT * FROM contributions WHERE cycle_id=$1 AND user_id=$2', [activeCycle.id, req.user.id]);
      myContribution = contrib.rows[0] || null;
    }

    res.json({ ...group.rows[0], members: members.rows, cycles: cycles.rows, activeCycle, myContribution, isAdmin: group.rows[0].admin_id === req.user.id });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    const g = await client.query(
      ...
      [
        ...
        late_penalty || 0, payout_method || 'order', req.user.id, inviteCode
      ]
    );
    
    const group = g.rows[0];
    await client.query('INSERT INTO members (group_id,user_id,position) VALUES ($1,$2,$3)', [group.id, req.user.id, 1]);
    await client.query('INSERT INTO cycles (group_id,cycle_number,payout_user_id,start_date,status) VALUES ($1,1,$2,NOW(),$3)', [group.id, req.user.id, 'active']);
...
router.get('/:id/requests', authenticateToken, async (req, res) => {
  try {
    const group = await pool.query('SELECT * FROM groups WHERE id=$1 AND admin_id=$2', [req.params.id, req.user.id]);
  ...
  try {
    ...
    try {
      ...
      const request = await client.query(
        ...
        ['approved', new Date(), req.user.id, req.params.requestId, req.params.id]
      );
...
router.post('/:id/requests/:requestId/reject', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE member_requests SET status=$1, reviewed_at=$2, reviewed_by=$3 WHERE id=$4 AND group_id=$5 RETURNING *',
      ['rejected', new Date(), req.user.id, req.params.requestId, req.params.id]
    );
...
router.post('/:id/chat', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const user = await pool.query('SELECT name, avatar_color FROM users WHERE id=$1', [req.user.id]);
